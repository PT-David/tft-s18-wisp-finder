...
仙灵查询的用户回合范围从 Stage 2 开始；Stage 1 PvE 回合不属于可购买仙灵查询语义。
...
高级筛选可选择一个参考仙灵 A。仅当用户填写“精确回合”时，才生成剩余查询区间：

`A 的出现区间 ∩ [当前回合, +∞)`

阶段范围查询的 `rangeStart` 只定义范围过滤，不得隐式充当“当前回合”。未填写精确回合时，参考过滤使用 A 的完整出现区间。
...
参考仙灵使用按中英文名称检索的 autocomplete 选择，并支持从结果卡片直接设为参考；不使用包含全集的长下拉框。切换版本时必须清除 reference 与手动排除状态，卡片缓存也必须按 patch 隔离。
