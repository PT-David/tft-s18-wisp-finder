...
for (const group of expected.synonyms.queryExpansionGroups) for (const alias of group.aliases) {
  if (forbidden.has(alias.normalize('NFKC').toLocaleLowerCase())) errors.push(`rejected alias is materialized: ${alias}`);
}
