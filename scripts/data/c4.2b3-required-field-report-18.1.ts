...
import { composedReadiness, type Json } from './lib/c4.2b3-required-fields';
...
export function render(frozen:Json,b2Frozen:Json,b2:Json,decisions:Json){
 ...
 const readiness=composedReadiness(b2Frozen,b2,decisions),ready=readiness.filter((r:Json)=>r.status==='READY').length,blockers=readiness.reduce((n:number,r:Json)=>n+r.requiredBlockers.length,0);
...
| --- | --- | --- |
${readiness.map((r:Json)=>`| ${r.nameEn} (\`${r.communityDragonId}\`) | **${r.status}** | ${r.requiredBlockers.join(', ')||'—'} |`).join('\n')}
