...
export const GENERATOR_VERSION = 'c2.1-v1';
...
export const TAXONOMY: TaxonomyEntry[] = [
  ...
  T('crowd_control', '控制', '施加眩晕等群体控制。'), T('enemy_death', '敌方阵亡', '由敌方单位阵亡触发。', 'death_kill_execute'),
  T('execute_threshold', '处决阈值', '在目标低于生命阈值时处决。', 'death_kill_execute'), T('free_reroll', '免费刷新', '明确提供免费商店刷新。', 'reroll_vs_refresh'),
  T('gold_cost', '金币花费', '支付、失去或要求持有金币。', 'gold_cost_vs_reward'), T('gold_gain', '获得金币', '奖励或产出金币。', 'gold_cost_vs_reward'),
  ...
  T('shop_reroll', '商店刷新', '明确刷新 TFT 商店。', 'reroll_vs_refresh'), T('survival_duration', '存活持续时间', '按存活或持续秒数触发。', 'survival_vs_once_per_game'),
  T('temporary_item', '临时装备', '明确为临时且可装备或明确是装备的对象。', 'item_semantics'), T('time_stage', '时间或阶段', '由回合、阶段或战斗时间决定。'),
...
const R: Rule[] = [
  ...
  { key: 'gold_cost', regex: /失去\d+金币|花费\d+金币|支付\d+金币|至少拥有\d+金币|金币少于\d+|拥有足够的金币|费用在\d+到\d+金币/, risk: 'gold_cost_vs_reward', confidence: 'needs_review', reviewReason: '持有条件、售价和实际支付可能是不同的金币语义。', decision: '确认应归入金币花费，还是仅保留为持有/售价条件。' },
  ...
  { key: 'survival_duration', regex: /每有\d+名存活的友军|弈子在战斗中存活|在\d+秒后|持续\d+秒/ },
  ...
  { key: 'time_stage', regex: /准备阶段|回合|秒后|持续时间/ },
];
...
export interface QueryGroup { key: string; canonicalTerm: string; conceptKeys: string[]; aliases: { term: string; language: string }[]; evidence: string; collisionRisk: string[]; reviewStatus: 'draft_candidate' | 'needs_review' }
const GROUPS = ([
  { key: 'champion_duplicator_terms', canonicalTerm: 'Champion Duplicator', conceptKeys: ['champion_duplicator'], aliases: [{ term: 'Champion Duplicator', language: 'en' }, { term: '复制器', language: 'zh' }, { term: '妮蔻', language: 'game_slang' }], evidence: 'C2 canonical query vocabulary rule', collisionRisk: ['clone_vs_duplicator'], reviewStatus: 'needs_review' },
  { key: 'experience_terms', canonicalTerm: '经验', conceptKeys: ['xp_gain'], aliases: [{ term: '经验', language: 'zh' }, { term: 'XP', language: 'en' }], evidence: 'C2 canonical query vocabulary rule', collisionRisk: [], reviewStatus: 'draft_candidate' },
  { key: 'health_terms', canonicalTerm: '生命值', conceptKeys: ['player_health_gain', 'player_health_loss', 'player_health_threshold'], aliases: [{ term: 'HP', language: 'en' }, { term: '生命值', language: 'zh' }, { term: '血量', language: 'game_slang' }], evidence: 'C2 canonical query vocabulary rule', collisionRisk: ['player_vs_unit_health'], reviewStatus: 'needs_review' },
  { key: 'reroll_terms', canonicalTerm: '刷新', conceptKeys: ['shop_reroll', 'free_reroll'], aliases: [{ term: 'D', language: 'game_slang' }, { term: 'reroll', language: 'en' }, { term: 'roll', language: 'game_slang' }, { term: '刷新', language: 'zh' }], evidence: 'C2 canonical query vocabulary rule', collisionRisk: ['reroll_vs_refresh', 'single_letter_alias'], reviewStatus: 'needs_review' },
...
export function detectAliasCollisions(groups: readonly QueryGroup[]) {
  ...
  return [...aliases].filter(([, x]) => x.groups.size > 1 || x.concepts.size > 1).map(([alias, x]) => ({ alias, groupKeys: [...x.groups].sort(), conceptKeys: [...x.concepts].sort(), reviewStatus: 'needs_review' as const })).sort((a, b) => a.alias.localeCompare(b.alias));
}
...
export function generateSearchLexicon(dataset: WispDataset, inputBytes: Buffer) {
  ...
  const collisions = detectAliasCollisions(queryExpansionGroups);
  ...
  const synonymDraft = { ...common, queryExpansionGroups, recordAliases: [], collisions };
  const high = assignments.filter(a => a.confidence === 'candidate_high_confidence').length;
  const report = { ...common, summary: { recordsScanned: dataset.records.length, conceptCandidateAssignments: assignments.length, conceptKeysUsed: new Set(assignments.map(a => a.conceptKey)).size, queryExpansionGroups: queryExpansionGroups.length, recordAliases: 0, highConfidenceCandidates: high, needsReviewCandidates: assignments.length - high, collisionRiskItems: collisions.length + queryExpansionGroups.filter(g => g.collisionRisk.length).length }, reviewGroups: { player_vs_unit_health: review.player_vs_unit_health, death_kill_execute: review.death_kill_execute, item_semantics: review.item_semantics, gold_cost_vs_reward: review.gold_cost_vs_reward, clone_vs_duplicator: review.clone_vs_duplicator, reroll_vs_refresh: review.reroll_vs_refresh, other_ambiguous: [...review.survival_vs_once_per_game, ...review.other_ambiguous] }, synonymCollisions: collisions };
