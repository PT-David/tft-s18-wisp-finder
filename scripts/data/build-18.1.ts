...
function requirementFacts(values: unknown): Array<{ type: string; operator?: string; value?: number | boolean }> {
  const joined = (Array.isArray(values) ? values : []).map((value) => typeof value === 'string' ? value : String((value as Raw).textEn ?? (value as Raw).textZh ?? '')).join('; ').toLowerCase();
  const facts: Array<{ type: string; operator?: string; value?: number | boolean }> = [];
  const threshold = (type: string, pattern: RegExp) => { const match = joined.match(pattern); if (match) facts.push({ type, operator: '>=', value: Number(match[1]) }); };
  threshold('gold', /(?:at least|至少拥有?)\s*(\d+)\s*(?:gold|金币)/i);
  threshold('player_health', /(?:at least|生命值(?:高于|至少)|失去前至少拥有?)\s*(\d+)\s*(?:player health|health|生命值)?/i);
  threshold('level', /(?:level|等级(?:达到)?)\s*(\d+)/i);
  threshold('board_count', /(?:at least|至少|不少于)\s*(\d+)\s*(?:[^;]{0,20})(?:on board|在场|弈子)/i);
...
async function main() {
  ...
  const manualReview = {
    ...
    groups: {
      ...
      requirements_semantics: [...lolchessFieldAudit.requirements.presenceConflict, ...lolchessFieldAudit.requirements.structuredConflict].map((item) => ({ sourceRecords: item, candidateMapping: item.identity, evidence: 'Confirmed identity with Requirements presence or structured-fact disagreement.', whyNotAutoConfirmed: 'Cross-language free text is not safely machine-equivalent.', whatHumanMustDecide: 'Confirm requirement presence and semantics.' })),
