...
async function main() {
  ...
  const manifestBrowserSource = { sourceId: lolchessImported?.sourceId, tier: 'C', url: lolchessImported?.url, locale: lolchessImported?.locale, retrievedAt: lolchessImported?.retrievedAt, pageUpdatedAt: lolchessImported?.pageUpdatedAt, sha256: lolchessImported?.sha256, recordCount: lolchessImported?.recordCount, fieldCoverage: lolchessImported?.fieldCoverage, fetchStatus: lolchessImported?.fetchStatus, useFor: ['identity_cross_check','cost_cross_check','effect_cross_check','stageRanges_cross_check','blossom_cross_check','prismatic_cross_check','requirements_cross_check','oncePerGame','reofferCooldownShops'], confidence: 'verified_third_party' };
  const targetedPaths = { source: 'data/raw/18.1/20260903/opgg-zh-cn.html', headers: 'data/raw/18.1/20260903/opgg-zh-cn.headers', parsed: 'data/raw/18.1/20260903/opgg-final-blockers.json' };
  const [targetedSource, targetedHeaders, targetedParsed] = await Promise.all(Object.values(targetedPaths).map((path) => readFile(resolve(root, path), 'utf8')));
  const targetedOpggSource = {
    sourceId: 'opgg_set18_wisps_20260903_targeted', tier: 'C', url: 'https://op.gg/zh-cn/tft/set/18', locale: 'zh_cn',
    retrievedAt: '2026-09-03T11:27:39Z', upstreamModifiedAt: 'unknown', confidence: 'community_high_confidence',
    useFor: ['category','category_cross_check','requirements','requirements_cross_check','cost','cost_cross_check'],
    sourceArtifact: targetedPaths.source, sha256: createHash('sha256').update(targetedSource).digest('hex'),
