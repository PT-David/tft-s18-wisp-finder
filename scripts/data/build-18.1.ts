...
async function main() {
  ...
  const dataTftUnmatched = records.filter(({ id }) => !reconciliation.matchedProduction.has(id)).map(({ id, nameEn, nameZh, riotId }) => ({ id, nameEn, nameZh, riotId }));
  ...
  const lolchessFieldAudit = {
    ...
    blossom: { productionCount: coverage.withBlossom, lolchessCount: lolchessRows.filter((row) => row.blossom).length, matchedAgreement: lolchessMatched.filter(({ record, row }) => Boolean(record.effects.blossom) === Boolean(row.blossom)).length, mismatches: lolchessMatched.filter(({ record, row }) => Boolean(record.effects.blossom) !== Boolean(row.blossom)).map(({ record, row }) => ({ nameEn: record.nameEn, productionHasBlossom: Boolean(record.effects.blossom), lolchessHasBlossom: Boolean(row.blossom) })) },
    requirements: { productionCount: coverage.withRequirements, lolchessCount: lolchessRows.filter((row) => Array.isArray(row.requirements) && row.requirements.length).length },
  ...
  const beforeDuplicateAudit = { confirmedIntersection: 152, exactClientKey: 144, candidateCount: 48, ambiguousCount: 27, unresolvedOpggCount: 48, dataTftOnlyCount: 17, confirmedCorpusButIncomplete: 8 };
