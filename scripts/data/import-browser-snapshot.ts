...
const snapshot = { sourceId: `${source}_set18_wisps_browser_import`, url: parsed.sourceUrl, locale: parsed.sourceUrl.match(/op\.gg\/([^/]+)/)?.[1] ?? 'en', importedAt, retrievedAt: importedAt, pageUpdatedAt: parsed.pageUpdatedAt, sha256: parsed.sha256, fetchStatus: 'browser_snapshot_imported', declaredRecordCount: parsed.declaredRecordCount, parsedRecordCount: parsed.records.length, recordCount: parsed.records.length, records: parsed.records };
